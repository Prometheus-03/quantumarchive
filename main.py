...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    if payload.message_id == 731853316383965194 and (isbeta()):
        channel = bot.get_guild(payload.guild_id).get_channel(payload.channel_id)
        message = await channel.fetch_message(payload.message_id)
        rcs = message.reactions
        user = payload.user_id
        emoji = payload.emoji.name
        database = DB("AutoRoles")
        database.set_collection("roles")
        everything = await database.find()
        autoroles = {i["emoji"]: i["roleid"] for i in everything}
        await bot.get_guild(payload.guild_id).get_member(user).add_roles(bot.get_guild(payload.guild_id).get_role(
            autoroles[emoji]))
        return
    try:
        channel = bot.get_guild(payload.guild_id).get_channel(payload.channel_id)
...
async def on_raw_reaction_remove(payload: discord.RawReactionActionEvent):
    if payload.message_id == 731853316383965194 and (isbeta()):
        ...
        everything = await database.find()
        autoroles = {i["emoji"]: i["roleid"] for i in everything}
        await bot.get_guild(payload.guild_id).get_member(user).remove_roles(bot.get_guild(payload.guild_id).get_role(
            autoroles[emoji]))
        return
    try:
        channel = bot.get_guild(payload.guild_id).get_channel(payload.channel_id)
...
if isbeta():
    ...
    bot.run("NzIxMzMzNDAxNDk5NTk4ODQ4.XwqoHA.o182MSYHE4gRhvx7EdV8ZV6Qtqk")
else:
    print("Stable bot running")
    bot.run("NzIwMjE2MzYxMzk3Mzg3Mjg0.Xwqn_g.LHvKexCqT975F79ypZ6ZQxVdmTM")
